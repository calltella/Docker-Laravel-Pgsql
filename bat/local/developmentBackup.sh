...
# セーブパスの結合
SAVEPATH="${USER_DIRECTORY}/Docker-Laravel-Pgsql/export/pgsql"

# ファイルネームを作成
SAVEFILE="development-dbdump-${CURRENT_DATE}"

# データベースをダンプ
# pg_dumpがファイルはバイナリファイルなので圧縮できない
docker exec $CONTAINER_ID bash -c 'set PGPASSWORD="${PGDUMP_PASSWORD}"'
docker exec $CONTAINER_ID bash -c "/usr/local/bin/pg_dump -U postgres -c --if-exists -d development -t phpipam_subnet_table -t phpipam_display_information -f /tmp/pgsql/${SAVEFILE}.sql"
